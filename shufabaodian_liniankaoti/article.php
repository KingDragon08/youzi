<?php
  ...
  $sql = "select * from youzi_article where Id=$Id and type=3";
  $article = $db->get_row($sql);
  if($article){
    $smarty->assign("title",$article->title);
    $smarty->assign("article",$article);
    $sql = "select Id,title,briefIntro,coverImg from youzi_article where type=3 and Id in (select Id from youzi_article where type=3 and Id>$Id) limit 10";
