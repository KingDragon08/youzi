<?php
/* Smarty version 3.1.30, created on 2017-09-05 11:08:58
  from "/Applications/XAMPP/xamppfiles/htdocs/youzi/youziribao/templates/index.html" */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.30',
  'unifunc' => 'content_59ae69aa862113_99323358',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    'd9ac326fbfa4a3fa820f2ec0c3e1c22ecb27c336' => 
    array (
      0 => '/Applications/XAMPP/xamppfiles/htdocs/youzi/youziribao/templates/index.html',
      1 => 1504602517,
      2 => 'file',
    ),
    '3a70bd3f78b75a7548fbf3d598da22f3d5a4c133' => 
    array (
      0 => '/Applications/XAMPP/xamppfiles/htdocs/youzi/nav2.html',
      1 => 1504592510,
      2 => 'file',
    ),
    '0618df8e7fd53082da5c4eb0a5af8a765b7a96fa' => 
    array (
      0 => '/Applications/XAMPP/xamppfiles/htdocs/youzi/footer.html',
      1 => 1504600236,
      2 => 'file',
    ),
  ),
  'cache_lifetime' => 3600,
),true)) {
function content_59ae69aa862113_99323358 (Smarty_Internal_Template $_smarty_tpl) {
?>
<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, height=device-height">
    <link rel="stylesheet" href="../assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="../assets/css/index.css" media="screen">
    <link rel="stylesheet" href="../assets/css/yingbi.css" media="screen">
    <link rel="stylesheet" href="../assets/css/style.css" media="screen">
    <title>优字日报</title>
</head>

<body>
    <div class="nav hidden-md hidden-sm hidden-xs" style="position:relative;">
  <div class="logo">LOGO</div>
  <ul>
    <li>
      <a href="../index.php">首页</a>
    </li>
    <li>
      <a href="../youziribao/" class="active">优字日报</a>
    </li>
    <li>
      <a href="../yingbijiaoxuezhongxin/" >硬笔教学中心</a>
      <div class="sub">
        <ul>
          <li>
            <a href="../yingbijiaoxuezhongxin/">教学用书与视频</a>
          </li>
          <li>
            <a href="../yingbijiaoxuezhongxin_youzishipinke/">优字视频课</a>
          </li>
        </ul>
      </div>
    </li>
    <li>
      <a href="../ruanbijiaoxuezhongxin/" >软笔教学中心</a>
      <div class="sub">
        <ul>
          <li>
            <a href="../ruanbijiaoxuezhongxin/">教学用书与视频</a>
          </li>
          <li>
            <a href="../ruanbijiaoxuezhongxin_youzishipinke/">优字视频课</a>
          </li>
        </ul>
      </div>
    </li>
    <li>
      <a href="../cepingandgerenzhongxin" >测评与个人中心</a>
    </li>
    <li>
      <a href="../youzirike" >优字日课</a>
    </li>
    <li>
      <a href="../mingjiashikong" >名家时空</a>
    </li>
    <li>
      <a href="../shufabaodian" >书法宝典</a>
      <div class="sub">
        <ul>
          <li>
            <a href="../shufabaodian/">书法常识</a>
          </li>
          <li>
            <a href="../shufabaodian_liniankaoti/">历年考题</a>
          </li>
          <li>
            <a href="../shufabaodian_mingjiajieshao/">名家介绍</a>
          </li>
        </ul>
      </div>
    </li>
    <li>
      <a href="../kunfenyumishequ" >昆粉玉米社区</a>
    </li>
  </ul>
</div>

    <div class="icp" style="background:#fff;">
        <div class="container">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <ol class="breadcrumb">
                    <li><a href="../index.php">首页</a></li>
                    <li><a href="javascript:;">优字日报</a></li>
                </ol>
            </div>
        </div>
    </div>
    <div class="category-post-section">
        <div class="container">
            <div class="row">

                            <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="post-box">
                        <div class="image-box">
                            <img src="images/post-3.jpg" alt="image">
                            <a href="#" class="add-sign-big color-navy-blue">
                            <img src="images/big-plus.png" alt="big-plus"/>
                        </a>
                        </div>
                        <ul class="comments-social">
                            <li class="dropdown">
                                <a href="#">
                                    <img src="images/more-icon.png">
                                </a>
                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <img src="images/qrcode.png">
                                    </li>
                                </ul>
                            </li>
                        </ul>
                        <div class="post-box-inner">
                            <a href="article.php?Id=7" class="box-read-more">
                            <img src="images/arrow.png" alt="arrow"/> 
                            了解更多
                            </a>
                            <div class="box-content">
                                <span>优字日报</span>
                                <a href="#" class="block-title">
                                优字日报标题
                            </a>
                                <p class="time">
                                    <i class="fa fa-clock-o"></i>
                                    2017-09-05
                                </p>
                                <p>
                                    优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简...
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                                <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="post-box">
                        <div class="image-box">
                            <img src="images/post-1.jpg" alt="image">
                            <a href="#" class="add-sign-big color-navy-blue">
                            <img src="images/big-plus.png" alt="big-plus"/>
                        </a>
                        </div>
                        <ul class="comments-social">
                            <li class="dropdown">
                                <a href="#">
                                    <img src="images/more-icon.png">
                                </a>
                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <img src="images/qrcode.png">
                                    </li>
                                </ul>
                            </li>
                        </ul>
                        <div class="post-box-inner">
                            <a href="article.php?Id=6" class="box-read-more">
                            <img src="images/arrow.png" alt="arrow"/> 
                            了解更多
                            </a>
                            <div class="box-content">
                                <span>优字日报</span>
                                <a href="#" class="block-title">
                                优字日报标题
                            </a>
                                <p class="time">
                                    <i class="fa fa-clock-o"></i>
                                    2017-09-05
                                </p>
                                <p>
                                    优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简...
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                                <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="post-box">
                        <div class="image-box">
                            <img src="images/post-6.jpg" alt="image">
                            <a href="#" class="add-sign-big color-navy-blue">
                            <img src="images/big-plus.png" alt="big-plus"/>
                        </a>
                        </div>
                        <ul class="comments-social">
                            <li class="dropdown">
                                <a href="#">
                                    <img src="images/more-icon.png">
                                </a>
                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <img src="images/qrcode.png">
                                    </li>
                                </ul>
                            </li>
                        </ul>
                        <div class="post-box-inner">
                            <a href="article.php?Id=5" class="box-read-more">
                            <img src="images/arrow.png" alt="arrow"/> 
                            了解更多
                            </a>
                            <div class="box-content">
                                <span>优字日报</span>
                                <a href="#" class="block-title">
                                优字日报标题
                            </a>
                                <p class="time">
                                    <i class="fa fa-clock-o"></i>
                                    2017-09-05
                                </p>
                                <p>
                                    优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简...
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                                <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="post-box">
                        <div class="image-box">
                            <img src="images/post-5.jpg" alt="image">
                            <a href="#" class="add-sign-big color-navy-blue">
                            <img src="images/big-plus.png" alt="big-plus"/>
                        </a>
                        </div>
                        <ul class="comments-social">
                            <li class="dropdown">
                                <a href="#">
                                    <img src="images/more-icon.png">
                                </a>
                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <img src="images/qrcode.png">
                                    </li>
                                </ul>
                            </li>
                        </ul>
                        <div class="post-box-inner">
                            <a href="article.php?Id=4" class="box-read-more">
                            <img src="images/arrow.png" alt="arrow"/> 
                            了解更多
                            </a>
                            <div class="box-content">
                                <span>优字日报</span>
                                <a href="#" class="block-title">
                                优字日报标题
                            </a>
                                <p class="time">
                                    <i class="fa fa-clock-o"></i>
                                    2017-09-05
                                </p>
                                <p>
                                    优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简...
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                                <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="post-box">
                        <div class="image-box">
                            <img src="images/post-4.jpg" alt="image">
                            <a href="#" class="add-sign-big color-navy-blue">
                            <img src="images/big-plus.png" alt="big-plus"/>
                        </a>
                        </div>
                        <ul class="comments-social">
                            <li class="dropdown">
                                <a href="#">
                                    <img src="images/more-icon.png">
                                </a>
                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <img src="images/qrcode.png">
                                    </li>
                                </ul>
                            </li>
                        </ul>
                        <div class="post-box-inner">
                            <a href="article.php?Id=3" class="box-read-more">
                            <img src="images/arrow.png" alt="arrow"/> 
                            了解更多
                            </a>
                            <div class="box-content">
                                <span>优字日报</span>
                                <a href="#" class="block-title">
                                优字日报标题
                            </a>
                                <p class="time">
                                    <i class="fa fa-clock-o"></i>
                                    2017-09-05
                                </p>
                                <p>
                                    优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简...
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                                <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="post-box">
                        <div class="image-box">
                            <img src="images/post-3.jpg" alt="image">
                            <a href="#" class="add-sign-big color-navy-blue">
                            <img src="images/big-plus.png" alt="big-plus"/>
                        </a>
                        </div>
                        <ul class="comments-social">
                            <li class="dropdown">
                                <a href="#">
                                    <img src="images/more-icon.png">
                                </a>
                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <img src="images/qrcode.png">
                                    </li>
                                </ul>
                            </li>
                        </ul>
                        <div class="post-box-inner">
                            <a href="article.php?Id=2" class="box-read-more">
                            <img src="images/arrow.png" alt="arrow"/> 
                            了解更多
                            </a>
                            <div class="box-content">
                                <span>优字日报</span>
                                <a href="#" class="block-title">
                                优字日报标题
                            </a>
                                <p class="time">
                                    <i class="fa fa-clock-o"></i>
                                    2017-09-05
                                </p>
                                <p>
                                    优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简...
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                                <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="post-box">
                        <div class="image-box">
                            <img src="images/post-1.jpg" alt="image">
                            <a href="#" class="add-sign-big color-navy-blue">
                            <img src="images/big-plus.png" alt="big-plus"/>
                        </a>
                        </div>
                        <ul class="comments-social">
                            <li class="dropdown">
                                <a href="#">
                                    <img src="images/more-icon.png">
                                </a>
                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <img src="images/qrcode.png">
                                    </li>
                                </ul>
                            </li>
                        </ul>
                        <div class="post-box-inner">
                            <a href="article.php?Id=1" class="box-read-more">
                            <img src="images/arrow.png" alt="arrow"/> 
                            了解更多
                            </a>
                            <div class="box-content">
                                <span>优字日报</span>
                                <a href="#" class="block-title">
                                优字日报标题
                            </a>
                                <p class="time">
                                    <i class="fa fa-clock-o"></i>
                                    2017-09-05
                                </p>
                                <p>
                                    优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简介优字日报简...
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                                
                
            </div>
        </div>
    </div>

    <ul class="pagination" style="margin-bottom: 20px;">
        <li style="display:inline-block;"><a href="#">&laquo;</a></li>
        <li class="active"><a href="#">1</a></li>
        <li><a href="#">2</a></li>
        <li><a href="#">3</a></li>
        <li><a href="#">4</a></li>
        <li><a href="#">5</a></li>
        <li><a href="#">&raquo;</a></li>
    </ul>

    <div class="footer">
    <div class="container">
        <nav class="mt20 mb10">友情连接</nav>
        <ul>
            <li><a href="#">友情链接1</a></li>
            <li><a href="#">友情链接2</a></li>
            <li><a href="#">友情链接3</a></li>
            <li><a href="#">友情链接4</a></li>
            <li><a href="#">友情链接5</a></li>
            <li><a href="#">友情链接6</a></li>
            <li><a href="#">友情链接7</a></li>
            <li><a href="#">友情链接8</a></li>
            <li><a href="#">友情链接9</a></li>
            <li><a href="#">友情链接1</a></li>
            <li><a href="#">友情链接2</a></li>
            <li><a href="#">友情链接3</a></li>
            <li><a href="#">友情链接4</a></li>
            <li><a href="#">友情链接5</a></li>
            <li><a href="#">友情链接6</a></li>
            <li><a href="#">友情链接7</a></li>
            <li><a href="#">友情链接8</a></li>
        </ul>
    </div>
    <div class="container">
        <nav class="mt20 mb10">合作伙伴</nav>
        <ul>
            <li><a href="#">合作伙伴1</a></li>
            <li><a href="#">合作伙伴2</a></li>
            <li><a href="#">合作伙伴3</a></li>
            <li><a href="#">合作伙伴4</a></li>
            <li><a href="#">合作伙伴5</a></li>
            <li><a href="#">合作伙伴6</a></li>
            <li><a href="#">合作伙伴7</a></li>
            <li><a href="#">合作伙伴8</a></li>
            <li><a href="#">合作伙伴9</a></li>
            <li><a href="#">合作伙伴1</a></li>
            <li><a href="#">合作伙伴2</a></li>
            <li><a href="#">合作伙伴3</a></li>
            <li><a href="#">合作伙伴4</a></li>
            <li><a href="#">合作伙伴5</a></li>
        </ul>
    </div>
    <hr>
    <p class="icp">CopyRight&copy;我是版权所属单位的名字 2016～2020</p>
    <p class="icp">京ICP备14045086号-1</p>
</div>
</body>

</html><?php }
}
